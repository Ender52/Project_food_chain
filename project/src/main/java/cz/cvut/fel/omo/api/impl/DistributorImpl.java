...
public class DistributorImpl extends PartyImpl implements Shop {
    ...
    public void checkRequestsToMe() {
        try {
            ...
            //if (myProduction.getMyStorage().has(ProductType.MILK, 5)) myProduction.getMyStorage().takeProducts(ProductType.MILK, 20);
            //if (myProduction.getMyStorage().has(ProductType.ORANGE, 5)) myProduction.getMyStorage().takeProducts(ProductType.ORANGE, 5);
//            if(!myProduction.myStorage.has(ProductType.SAUSAGE,100)) createRequest(ProductType.SAUSAGE, 100);
    ...
    public void sellProduct(ProductType p, int amount) {
        ...
        if (size < amount) {
            ...
            System.out.println("SHOP BUY " + size + " " + p + "s");
        } else {
            Product[] productsToSell = new Product[0];
            try {
            ...
            for (Product product : productsToSell) createOperation("Take", product);
            wallet.add(productsToSell[0].myPrice.amount * productsToSell.length);
            System.out.println("SHOP BUY " + amount + " " + p + "s");
        }
    }
